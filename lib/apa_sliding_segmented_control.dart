...
import 'apa_extensions.dart';


import 'apa_image_loading.dart';
//import 'colors.dart';
...
const double _kMinSegmentedControlHeight = 30.0;

const Color _kSeparatorColor = Colors.white;
...
class ApaSlidingSegmentedControl<T> extends StatefulWidget {
  ...
  @override
  SegmentedControlState<T> createState() => SegmentedControlState<T>();
}

class SegmentedControlState<T> extends State<ApaSlidingSegmentedControl<T>>
    with TickerProviderStateMixin<ApaSlidingSegmentedControl<T>> {
  ...
  @override
  void didUpdateWidget(ApaSlidingSegmentedControl<T> oldWidget) {
      ...
      builder: (BuildContext context, Widget child) {
        ...
        for ( T currentKey in keys) {
          ...
          final Widget child = DefaultTextStyle(
            ...
            child: Semantics(
              ...
              onTap: () {
                ...
                children.removeAt(widget.children.length);
                setState(() {
                  widget.isError = false;
                  widget.defaultOfView = false;
                });
          ...
          if(counter == widget.children.length && !isAdd && widget.defaultOfView) {
            children.add(widget.isError ? Align(alignment:AlignmentDirectional.centerStart ,child: ApaLoadImage(assetName: 'assets/svg/key_red.svg',)) : Align(alignment:AlignmentDirectional.centerStart ,child: ApaLoadImage(assetName: 'assets/svg/key.svg',)) ,);
            isAdd = true;
            counter = 0;
          }
        }
        ...
        final bool isDefaultView = widget.defaultOfView == null ? false : widget.defaultOfView;
        final bool isError = widget.isError == null ? false : widget.isError;
        final bool isDisable = widget.isDisable == null ? false : widget.isDisable;

        final Widget boxEnable = _SegmentedControlRenderEnableWidget<T>(
        ...
        final Widget boxDisable = _SegmentedControlRenderDisableWidget<T>(
          thumbRadius: Radius.circular(widget.cornerRadius),
          children: children,
          selectedIndex: selectedIndex,
          thumbColor: _kThumbColor,
        ...
        return UnconstrainedBox(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              ...
              border: Border.all(color: widget.isError ? "#E30000".toColor() : "#E8E8EF".toColor() ,
              ),
            ),
            child: isDisable ? boxEnable : boxDisable ,
          ),
        );
      },
    );
...
class _SegmentedControlRenderDisableWidget<T> extends MultiChildRenderObjectWidget {
  _SegmentedControlRenderDisableWidget( {
    Key key,
  ...
  final SegmentedControlState<T> state;
  ...
  void updateRenderObject(
      BuildContext context, RenderSegmentedControl<T> renderObject) {
    renderObject
      ..thumbColor = thumbColor
...
class _SegmentedControlRenderEnableWidget<T> extends MultiChildRenderObjectWidget {
  _SegmentedControlRenderEnableWidget( {
    Key key,
    List<Widget> children = const <Widget>[],
    @required this.selectedIndex,
    @required this.thumbColor,
...
class RenderSegmentedControl<T> extends RenderBox with ContainerRenderObjectMixin<RenderBox, ContainerBoxParentData<RenderBox>>, RenderBoxContainerDefaultsMixin<RenderBox, ContainerBoxParentData<RenderBox>> {
  ...
  //for click on null view
  bool ignoredForNullClick = false;
  ...
  void _onTapUp(TapUpDetails details) {
    if(ignoredForNullClick){
      ignoredForNullClick = false;
      return;
    }
    defaultView = false;
    isError = false;
  ...
  void _onDown(DragDownDetails details) {
    ...
    ///for click on null icon
    if( index == (childCount-1) && defaultView) {
      ignoredForNullClick = true;
      return;
    }
  ...
  void performLayout() {
    ...
    maxHeight = _kMinSegmentedControlHeight;
    ...
    //decrease the width for default view
    if(defaultView) {
      childWidth = math.min(childWidth, (constraints.maxWidth - totalSeparatorWidth) / childCount,) - (childWidth / childCount)  ;
    }else{
    ...
    while (child != null) {
      if(defaultView){
        final double boxHeight = child.getMaxIntrinsicHeight(childWidth + ((childWidth / childCount) ));
        maxHeight = math.max(maxHeight, boxHeight);child = childAfter(child);
      }else{
    ...
    // eventBus.on<SliderDisableEvent>().listen((data) {
    //   final type = data.disableView;
    //   isDisable = type;
    // });
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    if (highlightedIndex != null) {
      ...
      //for paint the box of selection
      if(!defaultView)
      ...
      //  for (int index = 0; index < childCount - 1; index += 1) {
      //_paintSeparator(context, offset, children[index]); //for now we don need it
      //  }
    }
  ...
  void _paintThumb(PaintingContext context, Offset offset, Rect thumbRect) {
    ...
    isDisable ? context.canvas.drawRRect(
      thumbRRect,
      Paint()
        ..shader = LinearGradient(
          ...
          stops: [0.0, 1],
        ).createShader(Rect.fromLTRB(0, 10.5, 0, thumbRect.height + 10)),
    ) : context.canvas.drawRRect(
      thumbRRect,
      Paint()
        ..shader = LinearGradient(
